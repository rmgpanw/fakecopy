...
class DummyDF:
    ...
    def __init__(self, df: pd.DataFrame = None, summary: pd.DataFrame = None):
        if df is not None:
            self.summary = self.summarize_dataframe(df)
        elif summary is not None:
            self.summary = summary
        else:
            raise ValueError("Either a DataFrame or a summary must be provided.")
    ...
    @staticmethod
    def create_dummy_data(seed: int = None) -> pd.DataFrame:
        """
        Creates a dummy DataFrame with predefined columns and random data.
